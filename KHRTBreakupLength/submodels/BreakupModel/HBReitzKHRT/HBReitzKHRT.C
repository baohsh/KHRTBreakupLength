...
#include "HBReitzKHRT.H"

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

template<class CloudType>
Foam::HBReitzKHRT<CloudType>::HBReitzKHRT
(
...
:
    ...
    weberLimit_(6.0),
    cb_(11),
    dInjector_(readScalar(this->coeffDict().lookup("Dinjector")))
...
template<class CloudType>
Foam::HBReitzKHRT<CloudType>::HBReitzKHRT(const HBReitzKHRT<CloudType>& bum)
:
...
template<class CloudType>
bool Foam::HBReitzKHRT<CloudType>::update
(
    const scalar dt,
    const vector& g,
    scalar& d,
...
)
{
    ...
    // breakup length to confine the RT instability 
    scalar Lb = cb_*sqrt(rho/rhoc)*dInjector_;
    ...
    if ((tc > tauRT) && (lambdaRT < d) && (lengthInjection > Lb))
    {
